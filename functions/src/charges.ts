 ...
 */
  ...
  */
  ...
  export const createCharge = async(uid: string, source: string, amount: number, email: string, idempotency_key: string) => {
  ...
  export const stripeCreateCharge = functions.https.onCall( async (data, context) => {
    ...
    return catchErrors( createCharge(uid, source, amount, email, idempotency_key) );
  });
